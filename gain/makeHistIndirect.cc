...
int main( int argc, char* argv[] ) {
  ...
  for(size_t i=0; i<files.size(); i++ ){
    ...
    ttree->SetBranchAddress("m_pmt", & m_pmt);
    ...
    for(int entry=0; entry<nentries; entry++  ) {
      ...
      // This is a bad bookeeping problem related to this batch of files,
      // it shoudn't be necessary in future
      if( m_optical_channel < 0 ){ continue; }
      ...
      m_pmts[m_pmt][i]->loadCharge( m_integral );
